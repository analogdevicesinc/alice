#!/usr/bin/python
# ADALM1000 alice-desktop 1.1.py(w) (1-18-2018)
# For Python version > = 2.7.8
# With external module pysmu ( libsmu = 0.88 or 0.89 for ADALM1000 )
# optional split I/O modes for Rev E,F hardware supported
# Created by D Mercer ()
#
import math
import time
...
root=Tk()
root.title("ALICE DeskTop 1.1 " + RevDate + ": ALM1000 Oscilloscope")
...
AWGBWave = 'dc'
#
DCV1 = DCV2 = MinV1 = MaxV1 = MinV2 = MaxV2 = MidV1 = PPV1 = MidV2 = PPV2 = SV1 = SI1 = SVA_B = 0
...
def BSaveConfig(filename):
    ...
    global MeasRMSV1, MeasRMSV2, MeasRMSI1, MeasRMSI2, MeasPhase, MeasRMSVA_B
    ...
    global FminEntry, MulXEntry, etswindow, ETSStatus, ETSDisp, ETSDir, ETSts, DivXEntry, FOffEntry, FminDisp
...
def BLoadConfig(filename):
    global TgInput, TgEdge, SingleShot, AutoLevel, HozPossentry, DevID
    global root, freqwindow, awgwindow, iawindow, xywindow, win1, win2
    global TRIGGERentry, TMsb, Xsignal, Ysignal, AutoCenterA, AutoCenterB
    global CHAsb, CHAIsb, CHBsb, CHBIsb, HScale, FreqTraceMode
    global CHAsbxy, CHAIsbxy, CHBsbxy, CHBIsbxy, HoldOffentry
...
#
def BReadData():
    global VBuffA, VBuffB, IBuffA, IBuffB, SHOWsamples, VFilterA, VFilterB
...
def BAbout():
    global RevDate, FWRevOne, HWRevOne, DevID
    # show info on software / firmware / hardware
    url = 'https://wiki.analog.com/_media/university/tools/m1k/alice/alice-desktop-1.1-setup.zip'
    try:
    ...
    showinfo("About ALICE", "ALICE DeskTop 1.1 " + RevDate + "\n" +
...
def BStart():
    global RUNstatus, PowerStatus, devx, PwrBt, DevID, FWRevOne
    if DevID == "No Device":
        ...
        if PowerStatus == 0:
            ...
            DevOne.ctrl_transfer( 0x40, 0x51, 49, 0, 0, 0, 100) # turn on analog power
        if (RUNstatus.get() == 0):
            RUNstatus.set(1)
        # pysmu.start(stopOnClose=True)
...
def BPower():
    ...
    if PowerStatus == 1:
        ...
        DevOne.ctrl_transfer( 0x40, 0x50, 49, 0, 0, 0, 100) # turn off analog power
    else:
        PowerStatus = 1
        PwrBt.config(bg="green",text="PWR-On")
        DevOne.ctrl_transfer( 0x40, 0x51, 49, 0, 0, 0, 100) # turn on analog power
...
def Analog_In():
    ...
    global IADisp, IAScreenStatus, CutDC, DevOne, AWGBMode, MuxEnb, BodeScreenStatus, BodeDisp, OhmDisp
    ...
    global ckb1, ckb2, ckb3, ckb4, ckb5, ckb6, OhmRunStatus, enb1, ETSDisp, VFilterA, VFilterB
    # Main loop
    while (True):
            ...
            if TimeDisp.get() > 0 or XYDisp.get() > 0:
                if MuxScreenStatus.get() == 0:
                    ...
                    if MuxSync.get() == 0:
                        ...
                        PIO3x = 0x51
                    if Show_CBD.get() == 1:
                        DevOne.ctrl_transfer(0x40, 0x51, 0, 0, 0, 0, 100) # set PIO 0 to 1
                        DevOne.ctrl_transfer(0x40, 0x51, 1, 0, 0, 0, 100) # set PIO 1 to 1
                        DevOne.ctrl_transfer(0x40, PIO2, 2, 0, 0, 0, 100) # set PIO 2 to 0
                        DevOne.ctrl_transfer(0x40, PIO3, 3, 0, 0, 0, 100) # set PIO 3 to sync pulse for sweep start
                        time.sleep(0.002)
                        DevOne.ctrl_transfer(0x40, PIO3x, 3, 0, 0, 0, 100) # set PIO 3 to return value
                        Analog_Time_In()
                        VBuffMD = VBuffB
                    if Show_CBC.get() == 1:
                        DevOne.ctrl_transfer(0x40, 0x50, 0, 0, 0, 0, 100) # set PIO 0 to 0
                        DevOne.ctrl_transfer(0x40, 0x51, 1, 0, 0, 0, 100) # set PIO 1 to 1
                        DevOne.ctrl_transfer(0x40, PIO2, 2, 0, 0, 0, 100) # set PIO 2 to 0
                        DevOne.ctrl_transfer(0x40, PIO3, 3, 0, 0, 0, 100) # set PIO 3 to sync pulse for sweep start
                        time.sleep(0.002)
                    ...
                    if Show_CBB.get() == 1:
                        DevOne.ctrl_transfer(0x40, 0x51, 0, 0, 0, 0, 100) # set PIO 0 to 1
                        DevOne.ctrl_transfer(0x40, 0x50, 1, 0, 0, 0, 100) # set PIO 1 to 0
                        DevOne.ctrl_transfer(0x40, PIO2, 2, 0, 0, 0, 100) # set PIO 2 to 0
                        DevOne.ctrl_transfer(0x40, PIO3, 3, 0, 0, 0, 100) # set PIO 3 to sync pulse for sweep start
                        time.sleep(0.002)
                        DevOne.ctrl_transfer(0x40, PIO3x, 3, 0, 0, 0, 100) # set PIO 3 to return value
                    ...
                    if Show_CBA.get() == 1 or ShowC1_V.get() == 1:
                        ...
                        DevOne.ctrl_transfer(0x40, 0x50, 1, 0, 0, 0, 100) # set PIO 1 to 0
                        DevOne.ctrl_transfer(0x40, PIO2, 2, 0, 0, 0, 100) # set PIO 2 to 0
                        DevOne.ctrl_transfer(0x40, PIO3, 3, 0, 0, 0, 100) # set PIO 3 to 1 sync pulse for sweep start
                        time.sleep(0.002)
                        DevOne.ctrl_transfer(0x40, PIO3x, 3, 0, 0, 0, 100) # set PIO 3 to return value
                        Analog_Time_In()
                        VBuffMA = VBuffB
...
#
def Ohm_Analog_In():
    global RMode, CHATestVEntry, CHATestREntry, CHA, CHB, DevOne, OhmA0, OhmA1
...
# Do input probe Calibration CH1VGain, CH2VGain, CH1VOffset, CH2VOffset
    ...
    if RMode.get() == 0:
        ...
        DevOne.ctrl_transfer(0x40, 0x50, 38, 0, 0, 0, 100) # set CHB GND switch to closed
...
# Do input probe Calibration CH1VGain, CH2VGain, CH1VOffset, CH2VOffset
    ...
    try:
        ...
        CHBVGainEntry.insert(0, InGainB)
    try:
        CurOffA = float(CHAIOffsetEntry.get())/1000.0 # convert to Amps
    except:
        CurOffA = 0.0
    try:
...
# Starting acquisition This is a HACK to get around non-continous AWG mode!
    if AWGScreenStatus.get() == 1: # don't try to start AWG if AWG screen is closed
        # restat AWGs if indicated
        if AWGSync.get() > 0:
            BAWGEnab()
    # 
    # time.sleep(0.001)
    ADsignal1 = DevOne.get_samples(SHOWsamples) # get samples for both channel A and B
    # waite to finish then return to open termination
    DevOne.ctrl_transfer( 0x40, 0x51, 32, 0, 0, 0, 100) # set CHA 2.5 V switch to open
    DevOne.ctrl_transfer( 0x40, 0x51, 33, 0, 0, 0, 100) # set CHA GND switch to open
    DevOne.ctrl_transfer( 0x40, 0x51, 37, 0, 0, 0, 100) # set CHB 2.5 V switch to open
    DevOne.ctrl_transfer( 0x40, 0x51, 38, 0, 0, 0, 100) # set CHB GND switch to open
    if AWGAIOMode.get() > 0: # Split Input / Output mode
        DevOne.ctrl_transfer( 0x40, 0x51, 34, 0, 0, 0, 100) # open voltage sense loop
    if AWGBIOMode.get() > 0: # Split Input / Output mode
        DevOne.ctrl_transfer( 0x40, 0x51, 39, 0, 0, 0, 100) # open voltage sense loop
...
# DC value = average of the data record
    ...
    DCI1 = numpy.mean(IBuffA[hldn:Endsample]) * 1000
    DCI2 = numpy.mean(IBuffB[hldn:Endsample]) * 1000
# find min and max values
    ...
    MinI1 = numpy.amin(IBuffA[hldn:Endsample]) * 1000
    MaxI1 = numpy.amax(IBuffA[hldn:Endsample]) * 1000
    MinI2 = numpy.amin(IBuffB[hldn:Endsample]) * 1000
    MaxI2 = numpy.amax(IBuffB[hldn:Endsample]) * 1000
# RMS value = square root of average of the data record squared
    SV1 = numpy.sqrt(numpy.mean(numpy.square(VBuffA[hldn:Endsample])))
    SI1 = numpy.sqrt(numpy.mean(numpy.square(IBuffA[hldn:Endsample]))) * 1000
    SV2 = numpy.sqrt(numpy.mean(numpy.square(VBuffB[hldn:Endsample])))
    SI2 = numpy.sqrt(numpy.mean(numpy.square(IBuffB[hldn:Endsample]))) * 1000
...
#
    ...
    if (RUNstatus.get() == 3) or (RUNstatus.get() == 4):
        if RUNstatus.get() == 3:
            RUNstatus.set(0)
            # pysmu.pause()      # Status is stopped 
        if RUNstatus.get() == 4:          
            RUNstatus.set(1)
            # pysmu.start(stopOnClose=True)# Status is (re)start
...
#
def MakeHistogram():
    global VBuffA, VBuffB, IBuffA, IBuffB, HBuffA, HBuffB, VFilterA, VFilterB
...
def FindRisingEdge():
    ...
    global VBuffA, VBuffB, IBuffA, IBuffB, hldn, SHOWsamples, VFilterA, VFilterB
...
def sel0(temp):
    ...
    # sending 0x50 = set to 0, 0x51 = set to 1
    if DAC0.get() == 1:
        DevOne.ctrl_transfer( 0x40, 0x50, 0, 0, 0, 0, 100) # set PIO 0 0
        DevOne.ctrl_transfer( 0x40, 0x50, 4, 0, 0, 0, 100) # set PIO 4 0
    elif DAC0.get() == 4:
        DevOne.ctrl_transfer( 0x40, 0x50, 0, 0, 0, 0, 100) # set PIO 0 0
        DevOne.ctrl_transfer( 0x40, 0x51, 4, 0, 0, 0, 100) # set PIO 4 1
    elif DAC0.get() == 2:
        DevOne.ctrl_transfer( 0x40, 0x50, 0, 0, 0, 0, 100) # set PIO 0 0
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 4, 0, 0, 1, 100) # set PIO 4 Z
    elif DAC0.get() == 3:
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 0, 0, 0, 1, 100) # set PIO 0 Z
        DevOne.ctrl_transfer( 0x40, 0x50, 4, 0, 0, 0, 100) # set PIO 4
    elif DAC0.get() == 5:
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 0, 0, 0, 1, 100) # set PIO 0 Z
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 4, 0, 0, 1, 100) # set PIO 4 Z
    elif DAC0.get() == 7:
        ...
        DevOne.ctrl_transfer( 0x40, 0x51, 0, 0, 0, 0, 100) # set PIO 0 1
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 4, 0, 0, 1, 100) # set PIO 4 Z
    elif DAC0.get() == 6:
        DevOne.ctrl_transfer( 0x40, 0x51, 0, 0, 0, 0, 100) # set PIO 0
        DevOne.ctrl_transfer( 0x40, 0x50, 4, 0, 0, 0, 100) # set PIO 4
    elif DAC0.get() == 9:
        DevOne.ctrl_transfer( 0x40, 0x51, 0, 0, 0, 0, 100) # set PIO 0
        DevOne.ctrl_transfer( 0x40, 0x51, 4, 0, 0, 0, 100) # set PIO 4
...
def sel1(temp):
    ...
    # sending 0x50 = set to 0, 0x51 = set to 1
    if DAC1.get() == 1:
        DevOne.ctrl_transfer( 0x40, 0x50, 1, 0, 0, 0, 100) # set PIO 0 0
        DevOne.ctrl_transfer( 0x40, 0x50, 5, 0, 0, 0, 100) # set PIO 4 0
    elif DAC1.get() == 4:
        DevOne.ctrl_transfer( 0x40, 0x50, 1, 0, 0, 0, 100) # set PIO 0 0
        DevOne.ctrl_transfer( 0x40, 0x51, 5, 0, 0, 0, 100) # set PIO 4 1
    elif DAC1.get() == 2:
        DevOne.ctrl_transfer( 0x40, 0x50, 1, 0, 0, 0, 100) # set PIO 0 0
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 5, 0, 0, 1, 100) # set PIO 4 Z
    elif DAC1.get() == 3:
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 1, 0, 0, 1, 100) # set PIO 0 Z
        DevOne.ctrl_transfer( 0x40, 0x50, 5, 0, 0, 0, 100) # set PIO 4
    elif DAC1.get() == 5:
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 1, 0, 0, 1, 100) # set PIO 0 Z
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 5, 0, 0, 1, 100) # set PIO 4 Z
    elif DAC1.get() == 7:
        ...
        DevOne.ctrl_transfer( 0x40, 0x51, 1, 0, 0, 0, 100) # set PIO 0 1
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 5, 0, 0, 1, 100) # set PIO 4 Z
    elif DAC1.get() == 6:
        DevOne.ctrl_transfer( 0x40, 0x51, 1, 0, 0, 0, 100) # set PIO 0
        DevOne.ctrl_transfer( 0x40, 0x50, 5, 0, 0, 0, 100) # set PIO 4
    elif DAC1.get() == 9:
        DevOne.ctrl_transfer( 0x40, 0x51, 1, 0, 0, 0, 100) # set PIO 0
        DevOne.ctrl_transfer( 0x40, 0x51, 5, 0, 0, 0, 100) # set PIO 4
...
def sel2(temp):
    ...
    # sending 0x50 = set to 0, 0x51 = set to 1
    if DAC2.get() == 1:
        DevOne.ctrl_transfer( 0x40, 0x50, 2, 0, 0, 0, 100) # set PIO 0 0
        DevOne.ctrl_transfer( 0x40, 0x50, 6, 0, 0, 0, 100) # set PIO 4 0
    elif DAC2.get() == 4:
        DevOne.ctrl_transfer( 0x40, 0x50, 2, 0, 0, 0, 100) # set PIO 0 0
        DevOne.ctrl_transfer( 0x40, 0x51, 6, 0, 0, 0, 100) # set PIO 4 1
    elif DAC2.get() == 2:
        DevOne.ctrl_transfer( 0x40, 0x50, 2, 0, 0, 0, 100) # set PIO 0 0
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 6, 0, 0, 1, 100) # set PIO 4 Z
    elif DAC2.get() == 3:
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 2, 0, 0, 1, 100) # set PIO 0 Z
        DevOne.ctrl_transfer( 0x40, 0x50, 6, 0, 0, 0, 100) # set PIO 4
    elif DAC2.get() == 5:
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 2, 0, 0, 1, 100) # set PIO 0 Z
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 6, 0, 0, 1, 100) # set PIO 4 Z
    elif DAC2.get() == 7:
        ...
        DevOne.ctrl_transfer( 0x40, 0x51, 2, 0, 0, 0, 100) # set PIO 0 1
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 6, 0, 0, 1, 100) # set PIO 4 Z
    elif DAC2.get() == 6:
        DevOne.ctrl_transfer( 0x40, 0x51, 2, 0, 0, 0, 100) # set PIO 0
        DevOne.ctrl_transfer( 0x40, 0x50, 6, 0, 0, 0, 100) # set PIO 4
    elif DAC2.get() == 9:
        DevOne.ctrl_transfer( 0x40, 0x51, 2, 0, 0, 0, 100) # set PIO 0
        DevOne.ctrl_transfer( 0x40, 0x51, 6, 0, 0, 0, 100) # set PIO 4
...
def sel3(temp):
    ...
    # sending 0x50 = set to 0, 0x51 = set to 1
    if DAC3.get() == 1:
        DevOne.ctrl_transfer( 0x40, 0x50, 3, 0, 0, 0, 100) # set PIO 0 0
        DevOne.ctrl_transfer( 0x40, 0x50, 7, 0, 0, 0, 100) # set PIO 4 0
    elif DAC3.get() == 4:
        DevOne.ctrl_transfer( 0x40, 0x50, 3, 0, 0, 0, 100) # set PIO 0 0
        DevOne.ctrl_transfer( 0x40, 0x51, 7, 0, 0, 0, 100) # set PIO 4 1
    elif DAC3.get() == 2:
        DevOne.ctrl_transfer( 0x40, 0x50, 3, 0, 0, 0, 100) # set PIO 0 0
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 7, 0, 0, 1, 100) # set PIO 4 Z
    elif DAC3.get() == 3:
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 3, 0, 0, 1, 100) # set PIO 0 Z
        DevOne.ctrl_transfer( 0x40, 0x50, 7, 0, 0, 0, 100) # set PIO 4
    elif DAC3.get() == 5:
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 3, 0, 0, 1, 100) # set PIO 0 Z
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 7, 0, 0, 1, 100) # set PIO 4 Z
    elif DAC3.get() == 7:
        ...
        DevOne.ctrl_transfer( 0x40, 0x51, 3, 0, 0, 0, 100) # set PIO 0 1
        Dval = DevOne.ctrl_transfer( 0xc0, 0x91, 7, 0, 0, 1, 100) # set PIO 4 Z
    elif DAC3.get() == 6:
        DevOne.ctrl_transfer( 0x40, 0x51, 3, 0, 0, 0, 100) # set PIO 0
        DevOne.ctrl_transfer( 0x40, 0x50, 7, 0, 0, 0, 100) # set PIO 4
    elif DAC3.get() == 9:
        DevOne.ctrl_transfer( 0x40, 0x51, 3, 0, 0, 0, 100) # set PIO 0
        DevOne.ctrl_transfer( 0x40, 0x51, 7, 0, 0, 0, 100) # set PIO 4
...
# 
    try:
        CurOffA = float(CHAIOffsetEntry.get())/1000 # convert to Amps
    except:
        CurOffA = 0.0
    try:
        CurOffB = float(CHBIOffsetEntry.get())/1000 # convert to Amps
    except:
        CurOffB = 0.0
    try:
        CurGainA = float(CHAIGainEntry.get())
            ...
            if t < TRACEsize:
                ...
                if MathTrace.get() > 0:
                    if MathTrace.get() == 1: # plot sum of CA-V and CB-V
                        ...
                        try:
                            Y1 = (IBuffB[t] / IBuffA[t]) * 1000  # current gain A to B
                        except:
                            Y1 = (IBuffB[t] / 0.000001) * 1000
...
def MakeTimeScreen():     # Update the screen with traces and text
    ...
    global ShowRA_V, ShowRA_I, ShowRB_V, ShowRB_I, ShowMath, MathUnits, MathXUnits, MathYUnits
    global Show_CBA, Show_CBB, Show_CBC, Show_CBD
...
def MakeXYScreen():
    ...
    global DacScreenStatus, DigScreenStatus
    global D0, D1, D2, D3, D4, D5, D6, D7
    global DevID, devx, MarkerXYNum, MarkerXYScale
    global HozPoss, HozPossentry
...
def onCanvasXYLeftClick(event):
    ...
    global HoldOffentry, Xsignal, Ysignal, COLORgrid, COLORtext
    global TMsb, CHAsbxy, CHBsbxy, CHAIsbxy, CHBIsbxy, MarkerXYScale
    ...
    #
        ...
        XIconv1 = float(GRWXY/10) / CH1IpdvRange
        XIconv2 = float(GRWXY/10) / CH2IpdvRange
        YIconv1 = float(GRHXY/10) / CH1IpdvRange
        YIconv2 = float(GRHXY/10) / CH2IpdvRange
        if MarkerXYScale.get() == 1:
            COLORmarker = COLORtrace1
        if MarkerXYScale.get() == 2:
            COLORmarker = COLORtrace2
        if MarkerXYScale.get() == 3:
            COLORmarker = COLORtrace3
        if MarkerXYScale.get() == 4:
            COLORmarker = COLORtrace4
        Yoffset1 = CHAOffset
        ...
        if Xsignal.get()==1 or Xsignal.get()==6:
            ...
            XUnits = " V"
        if Xsignal.get()==2:
            xvolts = ((xc1-event.x)/XIconv1) - CHAIOffset
            XUnits = " mA"
        if Xsignal.get()==3 or Xsignal.get()==7:
            xvolts = ((xc1-event.x)/Xconv2) - CHBOffset
            XUnits = " V"
        if Xsignal.get()==4:
            xvolts = ((xc1-event.x)/XIconv2) - CHBIOffset
            XUnits = " mA"
        if Ysignal.get()==1:
            yvolts = ((event.y-c1)/Yconv1) - CHAOffset
            YUnits = " V"
        if Ysignal.get()==2:
            yvolts = ((event.y-c1)/YIconv1) - CHAIOffset
            YUnits = " mA"
        if Ysignal.get()==3:
            yvolts = ((event.y-c2)/Yconv2) - CHBOffset
            YUnits = " V"
        if Ysignal.get()==4:
            yvolts = ((event.y-c1)/YIconv2) - CHBIOffset
            YUnits = " mA"
        if Xsignal.get()==5:
            if MathXAxis == "V-A":
                xvolts = ((xc1-event.x)/Xconv1) - CHAOffset
                ...
                xvolts = ((xc1-event.x)/XIconv1) - CHBIOffset
            else:
                xvolts = ((xc1-event.x)/Xconv1) - CHAOffset
            XUnits = MathXUnits
        if Ysignal.get()==5:
            if MathYAxis == "V-A":
                yvolts = ((event.y-c2)/Yconv1) - CHAOffset
            elif MathYAxis == "V-B":
                yvolts = ((event.y-c2)/Yconv2) - CHBOffset
            elif MathYAxis == "I-A":
                yvolts = ((event.y-c2)/YIconv1) - CHAIOffset
            elif MathYAxis == "I-B":
                yvolts = ((event.y-c2)/YIconv2) - CHBIOffset
            else:
                yvolts = ((event.y-c2)/Yconv2) - CHBOffset
            YUnits = MathYUnits
...
def find_smu_exe():
    ...
    path = 'C:\Program Files (x86)\Analog Devices\libsmu\\'
    name = 'smu.exe'
...
#def download_fw():
    ...
    #urllib2.urlretrieve ("https://github.com/analogdevicesinc/m1k-fw/blob/master/m1000.bin", "m1000.bin")
#
##def find_fw():
##
##    update = askyesno("WARNING","Board #1 Out of date Firmware Revision!\n Do you want to update the firmware? (Yes) or (No):\n")
##    if update:
##        while not os.path.isfile('m1000.bin'):
##            dload = askyesno("Download Firmware","Did not find m1000.bin file in current dir:\n Download from GitHub? (Yes) or (No):\n")
##            if dload:
##                urllib.urlretrieve ("https://github.com/analogdevicesinc/m1k-fw/raw/master/m1000.bin", "m1000.bin")
##                time.sleep(1)
##            else:
##                return
##        SmuExePath = find_smu_exe()
##        subprocess.call([SmuExePath, "-f", "m1000.bin" ], stdin=None, stdout=None, stderr=None, shell=True)
##        showwarning("WARNING","Un-plug board the plug in again to restart firmware")
##    else:
##        return
###
def ConnectDevice():
    global devx, DevID, DevOne, CHA, CHB, bcon, FWRevOne, HWRevOne
    global DevID2, DevTwo, CHC, CHD, FWRevTwo, HWRevTwo
    global DevID3, DevThree, CHE, CHF, FWRevThree, HWRevThree
    
    devx = Smu()
    brdnum = len(devx.devices)
    channum = len(devx.channels)
    if brdnum == 0:
    ...
    if brdnum > 0:
        ...
        try:
            FWRevOne = float(DevOne.fwver)
            if FWRevOne < 2.08:
                HWRevOne = "D"
            else:
                HWRevOne = DevOne.hwver
        except:
            FWRevOne = 0.0
            HWRevOne = "D"
            bcon.configure(text="Recon", style="RConn.TButton") # , bg="red")
            showwarning("WARNING","Board #1 Out of date Firmware Revision!")
            return
        bcon.configure(text="Conn", style="GConn.TButton")
        DevOne.ctrl_transfer(0x40, 0x51, 28, 0, 0, 0, 100) # blue off
        DevOne.ctrl_transfer(0x40, 0x50, 29, 0, 0, 0, 100) # green on
        DevOne.ctrl_transfer(0x40, 0x51, 47, 0, 0, 0, 100) # red off
    ...
    if brdnum > 1:
        ...
        CHD = devx.channels['D']
        CHD.set_mode('d')
        DevID2 = devx.serials[1]
        print DevID2
        DevTwo = devx.devices[1]
        FWRevTwo = float(DevTwo.fwver)
        HWRevTwo = DevTwo.hwver
        if FWRevTwo < 2.06: # Check firmware revision level > 2.06
            showwarning("WARNING","Board #2 Out of date Firmware Revision!")
        DevTwo.ctrl_transfer(0x40, 0x50, 28, 0, 0, 0, 100) # blue on
        DevTwo.ctrl_transfer(0x40, 0x51, 29, 0, 0, 0, 100) # green off
        DevTwo.ctrl_transfer(0x40, 0x51, 47, 0, 0, 0, 100) # red off
    if brdnum > 2:
        CHE = devx.channels['E']
        CHE.set_mode('d')
        CHF = devx.channels['F']
        CHF.set_mode('d')
        DevID3 = devx.serials[2]
        print DevID3
        DevThree = devx.devices[2]
        FWRevThree = float(DevThree.fwver)
        HWRevThree = DevThree.hwver
        if FWRevThree < 2.06: # Check firmware revision level > 2.06
            showwarning("WARNING","Board #3 Out of date Firmware Revision!")
        DevThree.ctrl_transfer(0x40, 0x51, 28, 0, 0, 0, 100) # blue off
        DevThree.ctrl_transfer(0x40, 0x51, 29, 0, 0, 0, 100) # green off
        DevThree.ctrl_transfer(0x40, 0x50, 47, 0, 0, 0, 100) # red on
#
...
def AWGAMakeMath():
    global AWGAwaveform, AWGSAMPLErate, VBuffA, VBuffB, IBuffA, IBuffB, VFilterA, VFilterB
...
def BAWGAModeLabel():
    global AWGAMode, AWGAIOMode, AWGAModeLabel, DevID, devx, DevOne, CHA, HWRevOne
    ...
    if AWGAIOMode.get() > 0: # Split Input / Output mode
        ...
        DevOne.ctrl_transfer( 0x40, 0x50, 34, 0, 0, 0, 100) # close voltage sense loop
...
def UpdateAWGA():
    ...
    global CHA, AWGSAMPLErate, DevID, devx, DevOne, HWRevOne
    ...
    if AWGATerm.get() == 0: # Open termination
        DevOne.ctrl_transfer( 0x40, 0x51, 32, 0, 0, 0, 100) # set 2.5 V switch to open
        DevOne.ctrl_transfer( 0x40, 0x51, 33, 0, 0, 0, 100) # set GND switch to open
    elif AWGATerm.get() == 1: # 50 Ohm termination to GND
        DevOne.ctrl_transfer( 0x40, 0x51, 32, 0, 0, 0, 100) # set 2.5 V switch to open
        DevOne.ctrl_transfer( 0x40, 0x50, 33, 0, 0, 0, 100) # set GND switch to closed
    elif AWGATerm.get() == 2: # 50 Ohm termination to +2.5 Volts
        DevOne.ctrl_transfer( 0x40, 0x50, 32, 0, 0, 0, 100) # set 2.5 V switch to closed
        DevOne.ctrl_transfer( 0x40, 0x51, 33, 0, 0, 0, 100) # set GND switch to open
    ...
    if AWGAIOMode.get() > 0: # Split Input / Output mode
        if HWRevOne == "D":
            if AWGAMode.get() == 0:
                AWGAMode.set(1)
                CHA.set_mode('i') # channel must be in source current mode
        DevOne.ctrl_transfer( 0x40, 0x51, 34, 0, 0, 0, 100) # open voltage sense loop
    else:
        DevOne.ctrl_transfer( 0x40, 0x50, 34, 0, 0, 0, 100) # close voltage sense loop
...
def AWGBMakeFourier():
    ...
    global AWGBDutyCyclevalue, AWGBFreqvalue, duty2lab
...
#
def AWGBMakeSSQ():
    global AWGBwaveform, AWGBAmplvalue, AWGBOffsetvalue, AWGBLength, AWGBPhaseDelay, duty2lab, phaseblab
...
def AWGBMakeUpDownRamp():
    global AWGBwaveform, AWGBAmplvalue, AWGBOffsetvalue, AWGBLength, AWGBPhaseDelay, duty2lab
    global AWGBFreqvalue, AWGBperiodvalue, AWGSAMPLErate, AWGBDutyCyclevalue, AWGBPhasevalue
...
def BAWGBModeLabel():
    ...
    if AWGBIOMode.get() > 0: # Split Input / Output mode
        ...
        DevOne.ctrl_transfer( 0x40, 0x50, 39, 0, 0, 0, 100) # close voltage sense loop
    label_txt = label_txt + " Mode"
...
def UpdateAWGB():
    ...
    if AWGBPhaseDelay.get() == 0:
        if AWGBWave == 'square':
            ...
            if AWGBPhasevalue > 359:
                AWGBPhasevalue = AWGBPhasevalue - 360.0
        if AWGBPhasevalue > 0:
    ...
    if AWGBTerm.get() == 0: # Open termination
        DevOne.ctrl_transfer( 0x40, 0x51, 37, 0, 0, 0, 100) # set 2.5 V switch to open
        DevOne.ctrl_transfer( 0x40, 0x51, 38, 0, 0, 0, 100) # set GND switch to open
    elif AWGBTerm.get() == 1: # 50 Ohm termination to GND
        DevOne.ctrl_transfer( 0x40, 0x51, 37, 0, 0, 0, 100) # set 2.5 V switch to open
        DevOne.ctrl_transfer( 0x40, 0x50, 38, 0, 0, 0, 100) # set GND switch to closed
    elif AWGBTerm.get() == 2: # 50 Ohm termination to +2.5 Volts
        DevOne.ctrl_transfer( 0x40, 0x50, 37, 0, 0, 0, 100) # set 2.5 V switch to closed
        DevOne.ctrl_transfer( 0x40, 0x51, 38, 0, 0, 0, 100) # set GND switch to open
    ...
    if AWGBWave == 'dc':
        ...
        elif AWGBMode.get() == 2: # Hi impedance mode
            CHB.set_mode('d')
            CHB.constant(0.0) #
    else:
        if AWGBMode.get() == 0: # Source Voltage measure current mode
    ...
    if AWGBIOMode.get() > 0: # Split Input / Output mode
        if HWRevOne == "D":
            if AWGBMode.get() == 0:
                AWGBMode.set(1)
                CHB.set_mode('i') # channel must be in source current mode
        DevOne.ctrl_transfer( 0x40, 0x51, 39, 0, 0, 0, 100) # open voltage sense loop
    else:
        DevOne.ctrl_transfer( 0x40, 0x50, 39, 0, 0, 0, 100) # close voltage sense loop
    ...
    while n < len(FSweepAdB):
        ...
        if ShowCA_VdB.get() == 1:
            if dB == 1:
                ...
                V = 7.079458 * math.sqrt(float(FFSweepAdB[n])) # scale to Vrms
            txt = txt + "," + str(V) 
        if ShowCB_VdB.get() == 1:
            if dB == 1:
                V = 10 * math.log10(float(FSweepBdB[n])) + 17  # Add 17 dB for max value of +10 dB
            else:
                V = 7.079458 * math.sqrt(float(FSweepBdB[n])) # scale to Vrms
            txt = txt  + "," + str(V)
        if ShowCA_P.get() == 1:
...
def BStartSA():
    global RUNstatus, PowerStatus, devx, PwrBt, freqwindow
...
#Bode Plot controls
def BStartBP():
    global RUNstatus, LoopNum, PowerStatus, devx, PwrBt, bodewindow
    ...
    if DevID == "No Device":
        ...
        if FSweepMode.get() == 3:
            AWGAMode.set(2) # Set AWG A to Hi-Z
            AWGBMode.set(2) # Set AWG B to Hi-Z
            MinigenMode.set(0) # Set MiniGen sahpe to Sine
        try:
...
def Analog_Freq_In():   # Read the audio from the stream and store the data into the arrays
    ...
    global CHA, CHB, DevOne, devx, MaxSamples
    ...
    if BodeDisp.get() == 0: # make new noise waveforms each sweep
        if AWGAShape.get() == 7 and AWGAMode.get() == 0:
            ...
            AWGBMakeUGNoise()
    if FSweepMode.get() > 0 and BodeDisp.get() > 0: # Run Sweep Gen only if selected and Bode display is active
    ...
    if BodeDisp.get() > 0: # check if doing Bode Plot
        if FStepSync.get() == 1: # output low - high - low pulse on PIO-0
            DevOne.ctrl_transfer( 0x40, 0x50, 0, 0, 0, 0, 100)
            DevOne.ctrl_transfer( 0x40, 0x51, 0, 0, 0, 0, 100)
            DevOne.ctrl_transfer( 0x40, 0x50, 0, 0, 0, 0, 100)
        ...
        if LoopNum.get() == 1 and FSweepSync.get() == 1: # output low - high - low pulse on PIO-1
            DevOne.ctrl_transfer( 0x40, 0x50, 1, 0, 0, 0, 100)
            DevOne.ctrl_transfer( 0x40, 0x51, 1, 0, 0, 0, 100)
            DevOne.ctrl_transfer( 0x40, 0x50, 1, 0, 0, 0, 100)
    ...
    while index < SHOWsamples:
        VAdata = ADsignal1[0][index][0] 
        FFTBuffA.append(VAdata)
        VBdata = ADsignal1[1][index][0] 
...
def MakeFreqTrace():        # Update the grid and trace
   ...
   # Vertical conversion factors (level dBs) and border limits
    ...
    Yphconv = float(GRHF) / 360.0
    Yp = float(Y0TF) + Yphconv + 180.0
    ...
    Fsample = float(AWGSAMPLErate / 2.0) / (TRACEsize - 1)       # Frequency step per sample   
    ...
    while n <= STOPsample:
        ...
        if ShowC1_P.get() == 1:
            ...
            RelPhase = RelPhase + PhErr - RelPhaseCorrection
...
def MakeBodeTrace():        # Update the grid and trace
    ...
    if FSweepMode.get() > 0 and len(FSweepAdB) > 4:
        ...
        Yphconv = float(GRHBP) / 360.0
        Yp = float(Y0TBP) + Yphconv + 180.0
        # Horizontal conversion factors (frequency Hz) and border limits
...
def MakeIAWindow():
    ...
    if IAScreenStatus.get() == 0:
        ...
        iawindow = Toplevel()
        iawindow.title("Impedance Analyzer 1.1 " + RevDate)
    ...
    while n < TRACEsize:
        ...
        if ShowC2_VdB.get() == 1:
            if dB == 1:
                ...
                V = 7.079458 * math.sqrt(float(FFTresultB[n])) # scale to Vrms
            txt = txt  + "," + str(V)
        if ShowC1_P.get() == 1:
            RelPhase = PhaseA[n]-PhaseB[n]
            if RelPhase > 180:
...
def MakeAWGWindow():
    ...
    if AWGScreenStatus.get() == 0:
        ...
        awgwindow = Toplevel()
        awgwindow.title("AWG 1.1 " + RevDate)
        awgwindow.resizable(FALSE,FALSE)
        awgwindow.geometry('+0+100')
        awgwindow.protocol("WM_DELETE_WINDOW", DestroyAWGScreen)
        #
        frame2 = LabelFrame(awgwindow, text="AWG CH A", style="A10R1.TLabelframe") # , fg=COLORtraceR1, font="Arial 10 bold", borderwidth=5, relief=RIDGE)
        frame2.pack(side=LEFT, expand=1, fill=Y)
        # 
        frame3 = LabelFrame(awgwindow, text="AWG CH B", style="A10R2.TLabelframe") # , fg=COLORtraceR2, font="Arial 10 bold", borderwidth=5, relief=RIDGE)
        ...
        ModeAMenu = Menubutton(awg1eb, text="Mode", style="W5.TButton") # , relief=RAISED)
        ...
        ShapeAMenu = Menubutton(awg1eb, text="Shape", style="W5.TButton") # , relief=RAISED)
        ShapeAMenu.menu = Menu(ShapeAMenu, tearoff = 0 )
        ShapeAMenu["menu"] = ShapeAMenu.menu
        ShapeAMenu.menu.add_radiobutton(label="DC", variable=AWGAShape, value=0, command=BAWGAShape)
        ShapeAMenu.menu.add_radiobutton(label="Sine", variable=AWGAShape, value=1, command=BAWGAShape)
        ShapeAMenu.menu.add_radiobutton(label="Triangle", variable=AWGAShape, value=2, command=BAWGAShape)
        ShapeAMenu.menu.add_radiobutton(label="Sawtooth", variable=AWGAShape, value=3, command=BAWGAShape)
        ShapeAMenu.menu.add_radiobutton(label="Square", variable=AWGAShape, value=4, command=BAWGAShape)
        ShapeAMenu.menu.add_radiobutton(label="StairStep", variable=AWGAShape, value=5, command=BAWGAShape)
        ...
        AWGAAmplEntry.bind("<Return>", BAWGAAmpl)
        ...
        AWGAOffsetEntry.bind("<Return>", BAWGAOffset)
        ...
        AWGAFreqEntry.bind("<Return>", BAWGAFreq)
        ...
        AWGAPhaseEntry.bind("<Return>", BAWGAPhase)
        ...
        AWGADutyCycleEntry.bind("<Return>", BAWGADutyCycle)
        ...
        ModeBMenu = Menubutton(awg2eb, text="Mode", style="W5.TButton") #, relief=RAISED)
        ...
        ShapeBMenu = Menubutton(awg2eb, text="Shape", style="W5.TButton") # , relief=RAISED)
        ShapeBMenu.menu = Menu(ShapeBMenu, tearoff = 0 )
        ShapeBMenu["menu"] = ShapeBMenu.menu
        ShapeBMenu.menu.add_radiobutton(label="DC", variable=AWGBShape, value=0, command=BAWGBShape)
        ShapeBMenu.menu.add_radiobutton(label="Sine", variable=AWGBShape, value=1, command=BAWGBShape)
        ShapeBMenu.menu.add_radiobutton(label="Triangle", variable=AWGBShape, value=2, command=BAWGBShape)
        ShapeBMenu.menu.add_radiobutton(label="Sawtooth", variable=AWGBShape, value=3, command=BAWGBShape)
        ShapeBMenu.menu.add_radiobutton(label="Square", variable=AWGBShape, value=4, command=BAWGBShape)
        ShapeBMenu.menu.add_radiobutton(label="StairStep", variable=AWGBShape, value=5, command=BAWGBShape)
        ...
        AWGBAmplEntry.bind("<Return>", BAWGBAmpl)
        ...
        AWGBOffsetEntry.bind("<Return>", BAWGBOffset)
        ...
        AWGBFreqEntry.bind("<Return>", BAWGBFreq)
        ...
        AWGBPhaseEntry.bind("<Return>", BAWGBPhase)
        ...
        AWGBDutyCycleEntry.bind("<Return>", BAWGBDutyCycle)
...
def MakeMuxModeWindow():
    ...
    if MuxScreenStatus.get() == 0 and DacScreenStatus.get() == 0 and DigScreenStatus.get() == 0:
        ...
        muxwindow = Toplevel()
        muxwindow.title("CH-B Mux 1.1 " + RevDate)
        ...
        SyncButton = Checkbutton(frameE, compound=TOP, image=hipulseimg, variable=MuxSync, command=SyncImage) # , width=25, height=25
...
def BStepSync():
    ...
    if FStepSync.get() == 0:
        Tval = DevOne.ctrl_transfer( 0xc0, 0x91, 0, 0, 0, 1, 100) # set PIO-0 to Z
...
def BSweepSync():
    ...
    if FSweepSync.get() == 0:
        Tval = DevOne.ctrl_transfer( 0xc0, 0x91, 1, 0, 0, 1, 100) # set PIO-1 to Z
...
def MakeBodeWindow():
    ...
    if BodeScreenStatus.get() == 0:
        ...
        AWGBMode.set(2) # Set AWG B to Hi-Z
        bodewindow = Toplevel()
        bodewindow.title("Bode Plotter 1.1 " + RevDate)
        ...
        FSweepmenu = Label(frame2bp, text="-Sweep Gen-", style="A10B.TLabel") # , font = "Arial 10 bold")
...
def MakeSpectrumWindow():
    ...
    if SpectrumScreenStatus.get() == 0:
        ...
        freqwindow = Toplevel()
        freqwindow.title("Spectrum Analyzer 1.1 " + RevDate)
...
def MakeXYWindow():
    ...
    if XYScreenStatus.get() == 0:
        ...
        xywindow = Toplevel()
        xywindow.title("X-Y Plot 1.1 " + RevDate)
...
#
def SelfCalibration():
    global DevID, DevOne, CHA, CHB, RevDate, OnBoardRes, AD584act, FWRevOne
    ...
    if FWRevOne < 2.06: # Check firmware revision level > 2.06
        ...
        return
    calwindow = Toplevel()
    calwindow.title("ALM1000 Calibration tool " + RevDate)
    # display wigets
    ...
    if askyesno("Reset Calibration", "Do You Need To Reset Default Calibration?", parent=calwindow):
        #print(DevOne.calibration)
        try:
            DevOne.write_calibration("calib_default.txt")
        except:
            filename = askopenfilename(defaultextension = ".txt", filetypes=[("Default Cal File", "*.txt")], parent=calwindow)
            DevOne.write_calibration(filename)
        #print(DevOne.calibration)
    #
    devidstr = DevID[17:31]
    ...
    DevOne.ctrl_transfer( 0x40, 0x50, 34, 0, 0, 0, 100) # close voltage sense loop just in case
    DevOne.ctrl_transfer( 0x40, 0x50, 39, 0, 0, 0, 100) # close voltage sense loop just in case
    ADsignal1 = []              # Ain signal array channel
    ADsignal1 = DevOne.get_samples(1010)
    ...
    DevOne.ctrl_transfer(0x40, 0x51, 37, 0, 0, 0, 100) # set CHB 2.5 V switch to open
    DevOne.ctrl_transfer(0x40, 0x51, 38, 0, 0, 0, 100) # set CHB GND switch to open
    showinfo("CONNECT","Connect External Voltage reference (AD584) to both CHA and CHB inputs.",  parent=calwindow)
    while (BadData):       # loop till good reading
        # Get A  and B AD584 data
        CHA.set_mode('d')
        CHB.set_mode('d')
        ADsignal1 = DevOne.get_samples(1010) # get samples for dev channel A0 and B0
        CHA584Raw = CHB584Raw = 0.0 # initalize measurment variable
        # get_samples returns a list of values for voltage [0] and current [1]
        for index in range(1000): # calculate average
            CHA584Raw += ADsignal1[0][index+10][0] # Sum for average CA voltage 
            CHB584Raw += ADsignal1[1][index+10][0] # Sum for average CB voltage

        CHA584Raw = CHA584Raw / 1000.0 # calculate average
        CHB584Raw = CHB584Raw / 1000.0 # calculate average
        VString = "CA 584 Volts " + ' {0:.4f} '.format(CHA584Raw) # format with 4 decimal places
        labelAMax.config(text = VString) # change displayed value
        VString = "CB 584 Volts " + ' {0:.4f} '.format(CHB584Raw) # format with 4 decimal places
        labelBMax.config(text = VString) # change displayed value
        if CHA584Raw < 2.2 or CHA584Raw > 2.8 or CHB584Raw < 2.2 or CHB584Raw > 2.8:
            showerror("CONNECT","Did not get good data from AD584 check connections and try again.", parent=calwindow)
        else:
            BadData = 0
    #
    #
    ...
    # get_samples returns a list of values for voltage [0] and current [1]
    for index in range(1000): # calculate average
        CHAGndRaw += ADsignal1[0][index+10][0] # Sum for average CA voltage 
        CHBGndRaw += ADsignal1[1][index+10][0] # Sum for average CB voltage
        CHAI0gRaw += ADsignal1[0][index+10][1] # Sum for average CA current 
        CHBI0gRaw += ADsignal1[1][index+10][1] # Sum for average CB current
    ...
    DevOne.ctrl_transfer(0x40, 0x50, 32, 0, 0, 0, 100) # set 2.5 V switch to closed
    DevOne.ctrl_transfer(0x40, 0x51, 33, 0, 0, 0, 100) # set GND switch to open
    DevOne.ctrl_transfer(0x40, 0x50, 37, 0, 0, 0, 100) # set CHB 2.5 V switch to closed
    DevOne.ctrl_transfer(0x40, 0x51, 38, 0, 0, 0, 100) # set CHB GND switch to open
    ADsignal1 = DevOne.get_samples(1010) # get samples for dev channel A0 and B0
    # get_samples returns a list of values for voltage [0] and current [1]
    for index in range(1000): # calculate average
        CHA2p5Raw += ADsignal1[0][index+10][0] # Sum for average CA voltage 
        CHB2p5Raw += ADsignal1[1][index+10][0] # Sum for average CB voltage
        CHAI02p5Raw += ADsignal1[0][index+10][1] # Sum for average CA current 
        CHBI02p5Raw += ADsignal1[1][index+10][1] # Sum for average CB current
    ...
    # get_samples returns a list of values for voltage [0] and current [1]
    for index in range(1000): # calculate average
        CHAF0vRaw += ADsignal1[0][index+10][0] # Sum for average CA voltage 
        CHBF0vRaw += ADsignal1[1][index+10][0] # Sum for average CB voltage
        CHAI0F0Raw += ADsignal1[0][index+10][1] # Sum for average CA current 
        CHBI0F0Raw += ADsignal1[1][index+10][1] # Sum for average CB current
    ...
    # get_samples returns a list of values for voltage [0] and current [1]
    for index in range(1000): # calculate average
        CHAF25vRaw += ADsignal1[0][index+10][0] # Sum for average CA voltage 
        CHBF25vRaw += ADsignal1[1][index+10][0] # Sum for average CB voltage
        CHAI0F25Raw += ADsignal1[0][index+10][1] # Sum for average CA current 
        CHBI0F25Raw += ADsignal1[1][index+10][1] # Sum for average CB current
    ...
    DevOne.ctrl_transfer(0x40, 0x51, 32, 0, 0, 0, 100) # set CHA 2.5 V switch to open
    DevOne.ctrl_transfer(0x40, 0x50, 33, 0, 0, 0, 100) # set CHA GND switch to closed
    DevOne.ctrl_transfer(0x40, 0x51, 37, 0, 0, 0, 100) # set CHB 2.5 V switch to open
    DevOne.ctrl_transfer(0x40, 0x50, 38, 0, 0, 0, 100) # set CHB GND switch to closed
    CHA.set_mode('V')
    CHA.constant(5.0)
    ...
    for index in range(1000): # calculate average
        CHASr5vRaw += ADsignal1[0][index+10][0] # Sum for average CA voltage 
        CHBSr5vRaw += ADsignal1[1][index+10][0] # Sum for average CB voltage
        CHAISr5vRaw += ADsignal1[0][index+10][1] # Sum for average CA current 
        CHBISr5vRaw += ADsignal1[1][index+10][1] # Sum for average CB current
    ...
    DevOne.ctrl_transfer(0x40, 0x50, 32, 0, 0, 0, 100) # set CHA 2.5 V switch to closed
    DevOne.ctrl_transfer(0x40, 0x51, 33, 0, 0, 0, 100) # set CHA GND switch to open
    DevOne.ctrl_transfer(0x40, 0x50, 37, 0, 0, 0, 100) # set CHB 2.5 V switch to closed
    DevOne.ctrl_transfer(0x40, 0x51, 38, 0, 0, 0, 100) # set CHB GND switch to open
    CHA.set_mode('V')
    ...
    for index in range(1000): # calculate average
        CHASr0vRaw += ADsignal1[0][index+10][0] # Sum for average CA voltage 
        CHBSr0vRaw += ADsignal1[1][index+10][0] # Sum for average CB voltage
        CHAISr0vRaw += ADsignal1[0][index+10][1] # Sum for average CA current 
        CHBISr0vRaw += ADsignal1[1][index+10][1] # Sum for average CB current
    ...
    for index in range(1000): # calculate average
        CHAVSr0iRaw += ADsignal1[0][index+10][0] # Sum for average CA voltage 
        CHBVSr0iRaw += ADsignal1[1][index+10][0] # Sum for average CB voltage
        CHAISr0iRaw += ADsignal1[0][index+10][1] # Sum for average CA current 
        CHBISr0iRaw += ADsignal1[1][index+10][1] # Sum for average CB current
    ...
    for index in range(1000): # calculate average
        CHAVSr100Raw += ADsignal1[0][index+10][0] # Sum for average CA voltage 
        CHBVSr100Raw += ADsignal1[1][index+10][0] # Sum for average CB voltage
        CHAISr100Raw += ADsignal1[0][index+10][1] # Sum for average CA current 
        CHBISr100Raw += ADsignal1[1][index+10][1] # Sum for average CB current
    ...
    for index in range(1000): # calculate average
        CHAVSrN45Raw += ADsignal1[0][index+10][0] # Sum for average CA voltage 
        CHBVSrN45Raw += ADsignal1[1][index+10][0] # Sum for average CB voltage
        CHAISrN45Raw += ADsignal1[0][index+10][1] # Sum for average CA current 
        CHBISrN45Raw += ADsignal1[1][index+10][1] # Sum for average CB current
...
def SPIShiftOut(DValue):
    ...
    if datalen < 16:
       ...
       datalen = len(datastr)
    i = 1
    DevOne.ctrl_transfer(0x40, 0x50, 0, 0, 0, 0, 100) # fsync to 0
    while i < datalen+1:
    # sending 0x50 = set to 0, 0x51 = set to 1
        D1code = 0x50 + int(datastr[i-1])
        DevOne.ctrl_transfer(0x40, D1code, 1, 0, 0, 0, 100) # data bit
        DevOne.ctrl_transfer(0x40, 0x51, 3, 0, 0, 0, 100) # sclk to 1
        DevOne.ctrl_transfer(0x40, 0x50, 3, 0, 0, 0, 100) # sclk to 0
        DevOne.ctrl_transfer(0x40, 0x51, 3, 0, 0, 0, 100) # sclk to 1
        i = i + 1
    DevOne.ctrl_transfer(0x40, 0x51, 0, 0, 0, 0, 100) # fsync to 1
#
def BSendMG():
...
def MakeMinigenWindow():
    ...
    if MinigenScreenStatus.get() == 0:
        ...
        minigenwindow.title("MiniGen 1.1 " + RevDate)
...
def DA1ShiftOut(D1Value, D2Value):
    ...
    i = 1
#
    DevOne.ctrl_transfer(0x40, 0x50, 0, 0, 0, 0, 100) # sync to 0
    while i < datalen+1:
    # sending 0x50 = set to 0, 0x51 = set to 1
        D1code = 0x50 + int(data1str[i-1])
        D2code = 0x50 + int(data2str[i-1])
        DevOne.ctrl_transfer(0x40, D1code, 1, 0, 0, 0, 100) # data 0 bit
        DevOne.ctrl_transfer(0x40, D2code, 2, 0, 0, 0, 100) # data 1 bit
        DevOne.ctrl_transfer(0x40, 0x51, 3, 0, 0, 0, 100) # sclk to 1
        DevOne.ctrl_transfer(0x40, 0x50, 3, 0, 0, 0, 100) # sclk to 0
        DevOne.ctrl_transfer(0x40, 0x51, 3, 0, 0, 0, 100) # sclk to 1
        i = i + 1
    DevOne.ctrl_transfer(0x40, 0x51, 0, 0, 0, 0, 100) # sync to 1
    #   
def BSendDA1():
...
def MakeDA1Window():
    ...
    if DA1ScreenStatus.get() == 0:
        ...
        da1window = Toplevel()
        da1window.title("DA1 PMOD 1.1 " + RevDate)
...
def DigPotShiftOut(DValue):
    ...
    if SingleDualPot.get() == 2: # send 8 bits of data
        if datalen < 8:
           ...
           datalen = len(datastr)
    i = 1
    DevOne.ctrl_transfer(0x40, 0x50, 3, 0, 0, 0, 100) # clock to 0
    DevOne.ctrl_transfer(0x40, 0x50, 0, 0, 0, 0, 100) # CS to 0
    while i < datalen+1:
    # CS --> PIO 0
    # D0 --> PIO 1
    # D1 --> PIO 2
    # SCLK --> PIO 3
        D1code = 0x50 + int(datastr[i-1])
        DevOne.ctrl_transfer(0x40, D1code, 1, 0, 0, 0, 100) # data bit
        DevOne.ctrl_transfer(0x40, 0x51, 3, 0, 0, 0, 100) # clock to 1
        DevOne.ctrl_transfer(0x40, 0x50, 3, 0, 0, 0, 100) # clock to 0
        i = i + 1
    DevOne.ctrl_transfer(0x40, 0x51, 0, 0, 0, 0, 100) # CS to 1
    #
def DigPotSend(Temp):
...
def BSendGS():
    ...
    global NumBitsEntry, DataBitsEntry, DevOne, SerDirection
...
def MakeETSWindow():
    ...
    if ETSStatus.get() == 0:
        ...
        ETSDisp.set(0)
        etswindow = Toplevel()
        etswindow.title("ETS Controls " + RevDate)
