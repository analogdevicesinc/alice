#!/usr/bin/python
# ADALM1000 DC Voltmeter Tool RR 6-6-2021
    ...
    while (True):
        if (RUNstatus.get() == 1):
            ...
            if not m1k_obj.IsContinuous():
                # print('Starting a cont session')
                m1k_obj.StartSession()
            m1k_obj.setmode('A', 'HI_Z')
            m1k_obj.setmode('B', 'HI_Z')
            # Get A0 and B0 data
            samples=m1k_obj.ContRead(100)
            # get_samples returns a list of values for voltage [0] and current [1]
            for sample in samples:
                DCVA += sample.A[0] # Sum for average CA voltage 
                DCVB += sample.B[0] # Sum for average CB voltage
            DCVA = DCVA / 100.0 # calculate average
            DCVB = DCVB / 100.0 # calculate average
...
m1k_obj.setchanterm('A', '2p5', 'OPEN') # set CHA 2.5 V switch to open
m1k_obj.setchanterm('A', 'GND', 'OPEN') # set CHA GND switch to open
